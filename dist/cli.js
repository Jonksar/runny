...
async function main(argv) {
    ...
    if (command === "doctor") {
        ...
        if (checks.some((c) => !c.ok && !c.warning))
            process.exitCode = 1;
        return;
    }
    if (command === "voices") {
    ...
    console.log(`Open: http://${hostname}:${args.port}/#token=${encodeURIComponent(args.token)}`);
    const phone = await tailscaleLink(args.port, args.token);
    if (!phone)
        console.log(`Phone: install Tailscale, run tailscale serve --bg ${args.port}, then open its HTTPS URL with /#token=${encodeURIComponent(args.token)}`);
    else if (phone.served)
        console.log(`Phone: ${phone.url}`);
    else
        console.log(`Phone: run tailscale serve --bg ${args.port} once, then open ${phone.url}`);
    let stopping = false;
    const stop = () => {
        if (stopping)
            return;
